...
var require_main2 = __commonJS({
  "node_modules/@netlify/otel/dist/main.cjs"(exports2, module2) {
    ...
    var getTracer = (name, version2) => {
      return globalThis[GET_TRACER]?.(name, version2);
    };
...
var require_utils = __commonJS({
  "netlify/functions/_utils.js"(exports2, module2) {
    var jwt = require_jsonwebtoken();
    var { connectLambda, getStore } = require_main3();
    function requireEnv(name) {
      const v = process.env[name];
      if (!v) throw new Error(`Missing ${name}`);
      return v;
    }
    function getDataStore2(event) {
    ...
    function json2(statusCode, body, extraHeaders = {}) {
      return {
        statusCode,
        headers: {
    ...
    async function readIndex2(store) {
      const idx = await store.get("leads:index", { type: "json" });
      return Array.isArray(idx) ? idx : [];
    }
    async function writeIndex2(store, ids) {
      await store.set("leads:index", ids, { type: "json" });
    }
    async function readLead2(store, id) {
      return store.get(`lead:${id}`, { type: "json" });
    }
    async function writeLead2(store, id, obj) {
      await store.set(`lead:${id}`, obj, { type: "json" });
    }
    async function deleteLead2(store, id) {
      await store.delete(`lead:${id}`);
    }
    function safeParseJson2(str) {
      try {
...
function normalizeStatus(s) {
  ...
  const allowed = /* @__PURE__ */ new Set(["lead", "follow_up", "appointment", "landed", "no"]);
  ...
  try {
    if (parts.length === 0) {
      if (event.httpMethod === "GET") {
        ...
        for (const id2 of ids) {
          const item = await readLead(store, id2);
      ...
      if (event.httpMethod === "POST") {
        ...
        await writeLead(store, id2, lead);
        const ids = await readIndex(store);
        ids.push(id2);
    ...
    if (parts.length === 1) {
      ...
      if (event.httpMethod === "PATCH") {
        ...
        const lead = await readLead(store, id);
        if (!lead) return json(404, { error: true, message: "Not found" }, cors);
        if (body.name !== void 0) lead.name = sanitizeString(body.name, 160);
        if (body.phone !== void 0) lead.phone = sanitizeString(body.phone, 60);
        if (body.email !== void 0) lead.email = sanitizeString(body.email, 120);
        if (body.industry !== void 0) lead.industry = sanitizeString(body.industry, 80);
        if (body.subIndustry !== void 0) lead.subIndustry = sanitizeString(body.subIndustry, 80);
        if (body.location !== void 0) lead.location = sanitizeString(body.location, 120);
